...
class ChatService {
  ...
  Future<void> downloadArtifact(String taskId, String artifactId) async {
    try {
      // Get the artifact data as binary
      final Uint8List bytes = await api.getBinary('agent/tasks/$taskId/artifacts/$artifactId');
      
      // For web, we'll need to handle this differently
      // TODO: Implement proper web download mechanism
      // For now, just log that the download was completed
      print('Artifact downloaded: $artifactId (${bytes.length} bytes)');
    } catch (e) {
